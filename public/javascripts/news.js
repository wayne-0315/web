function newnews (data) {
    ...
    `
                ...
                <div class="card-body">
                    <a href="/news-in?_id=${data._id}">
                    <img class="" src="../photos/${data.ipth}">
                    </a>
                    <h5 class="card-title">${data.title}</h5>
                    <a href="/news-in?_id=${data._id}" class="greenbtn btn btn-bottom-right">了解更多</a>
                </div>
    ...
    $.get("/getnews?" + search, function (res, status) {
    $('#type').val(res.type) ;
