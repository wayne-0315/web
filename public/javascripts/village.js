function newvillage (data) {
    ...
    `
                ...
                <div class="card-body">
                    <a href="/village-in?_id=${data._id}">
                    <img class="" src="../photos/${data.ipth}">
                    </a>
                    <h5 class="card-title">${data.title}</h5>
                    <a href="/village-in?_id=${data._id}" class="ywbtn btn btn-bottom-right">了解更多</a>
                </div>
    ...
    $.get("/getvillage?" + search, function (res, status) {
    $('#type').val(res.type) ;
    for (var i = 0; i < res.data.length; i++) {
    newvillage(res.data[i]);
    }
    });
    }
