function newbuy (data) {
    ...
    `
                ...
                <div class="card-body">
                    <a href="/buy-in?_id=${data._id}">
                    <img class="" src="../photos/${data.ipth}">
                    </a>
                    <h5 class="card-title">${data.title}</h5>
                    <p>${data.price}</p>
                    <a href="/buy-in?_id=${data._id}" class="pinkbtn btn btn-bottom-right">了解更多</a>
                </div>
    ...
    $.get("/getbuy?" + search, function (res, status) {
    $('#type').val(res.type) ;
